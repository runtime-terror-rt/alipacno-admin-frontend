...
export default function SettingsPage() {
  ...
  const [profile, setProfile] = useState<AdminProfileDetails>(ADMIN_PROFILE_DATA);
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [editForm, setEditForm] = useState<AdminProfileDetails>(ADMIN_PROFILE_DATA);

  const [modules, setModules] = useState(ROLE_MODULES);
  const [isCustomizing, setIsCustomizing] = useState<boolean>(false);

  const [integrations, setIntegrations] = useState<IntegrationItem[]>(CONNECTED_INTEGRATIONS);
  const [activities, setActivities] = useState<TimelineEvent[]>(ACTIVITY_TIMELINE);
  const [searchQuery, setSearchQuery] = useState<string>("");

  const [notifications, setNotifications] = useState<NotificationPreference[]>(NOTIFICATION_PREFERENCES);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-xs sm:text-sm mt-1 font-semibold">
            Manage your branch credentials, connected services, and notifications
          </p>
        </div>
      </div>
          ...
          <div className="bg-[#121214] border border-zinc-800 rounded-3xl p-6 relative overflow-hidden shadow-2xl">
            ...
            <div className="flex justify-between items-center pb-4 mb-6 border-b border-zinc-900/80 relative z-10">
              ...
              <div className="flex items-center space-x-2">
                ...
                <button 
                  onClick={() => alert("Actions list: Download Backup, System Diagnostic")}
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6 relative z-10">
              ...
              <div>
              <Image src="/branch-admin/admin.png" alt="Profile Avatar" width={100} height={100} className="rounded-full" />
            </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2 mt-3.5 text-xs text-zinc-400 font-semibold">
                    <div className="flex items-center justify-center sm:justify-start space-x-2.5">
                      ...
                      <span className="truncate hover:text-white transition-colors">{profile.email}</span>
                    </div>
                    <div className="flex items-center justify-center sm:justify-start space-x-2.5">
                      <Phone className="h-3.5 w-3.5 text-orange-500 shrink-0" />
                      <span className="hover:text-white transition-colors">{profile.phone}</span>
                    </div>
                    <div className="flex items-center justify-center sm:justify-start space-x-2.5">
            ...
            <div className="space-y-5 text-xs font-semibold">
              <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <h4 className="text-lg font-black text-white mt-1">{profile.role}</h4>
                </div>
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 pt-1">
                  {modules.map((mod) => (
                    ...
                    >
                      <div className={`h-4.5 w-4.5 rounded-md flex items-center justify-center border shrink-0 ${
                        mod.enabled 
                      ...
                      <span className="text-[10px] font-black uppercase tracking-wider truncate">{mod.name}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                if (item.logoType === "uber") {
                  brandMarkup = (
                    <div className="w-8 h-8 rounded-xl bg-black border border-zinc-850 flex flex-col items-center justify-center shrink-0">
                      <span className="text-[6px] font-black leading-none text-white tracking-widest">UBER</span>
                      <span className="text-[5px] font-bold text-emerald-450 mt-0.5 leading-none">Eats</span>
                    </div>
                  );
                  ...
                  brandMarkup = (
                    <div className="w-8 h-8 rounded-xl bg-[#00cdbc]/10 border border-[#00cdbc]/25 flex items-center justify-center shrink-0">
                      <span className="text-xs font-black text-[#00cdbc] tracking-widest">D</span>
                    </div>
                  );
                  ...
                  brandMarkup = (
                    <div className="w-8 h-8 rounded-xl bg-[#635bff]/10 border border-[#635bff]/25 flex items-center justify-center shrink-0">
                      <span className="text-[10px] font-black text-[#635bff] tracking-widest">S</span>
                    </div>
                  );
                  ...
                  brandMarkup = (
                    <div className="w-8 h-8 rounded-xl bg-[#f22f46]/10 border border-[#f22f46]/25 flex items-center justify-center shrink-0">
                      <span className="text-[7px] font-black text-[#f22f46]">twilio</span>
                    </div>
                  );
                ...
                return (
                  <div key={item.id} className="flex justify-between items-center py-3.5 first:pt-0 last:pb-0">
                    <div className="flex items-center space-x-3.5">
                      {brandMarkup}
                      <span className="text-white text-xs font-black tracking-wider uppercase">{item.name}</span>
                    </div>
            ...
            >
              ...
              <div className="mt-3">
                ...
                <span className="flex items-center space-x-1 mt-1 text-[8px] font-bold text-emerald-500">
                  ...
                  <span className="text-zinc-550 font-semibold">vs last period</span>
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 mb-5 border-b border-zinc-900/80 gap-3">
              ...
              <div className="relative">
                ...
                <span className="absolute left-2.5 top-2.5">
                  <svg className="w-2.5 h-2.5 text-zinc-550" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1 relative pl-3.5 border-l border-zinc-900">
              {filteredActivities.length > 0 ? (
                filteredActivities.map((act) => (
                  <div key={act.id} className="relative group text-xs font-semibold">
                    {/* Timeline bullet */}
      ...
      {isEditing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="w-full max-w-md bg-[#121214] border border-zinc-800 rounded-3xl p-6 space-y-6 shadow-2xl relative">
            ...
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-wider">Edit Admin Profile</h3>
              <p className="text-[11px] text-zinc-450 mt-1 font-semibold">Update Jesse Hayden's administrative details</p>
            </div>
            ...
            <div className="space-y-4 text-xs font-semibold">
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] uppercase tracking-widest text-zinc-400 font-bold">Email Address</label>
                <input 
                  type="email" 
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] uppercase tracking-widest text-zinc-400 font-bold">Phone Number</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] uppercase tracking-widest text-zinc-400 font-bold">Office Address</label>
                <textarea 
                  rows={2}
      ...
      {isCustomizing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="w-full max-w-lg bg-[#121214] border border-zinc-800 rounded-3xl p-6 space-y-6 shadow-2xl relative max-h-[85vh] flex flex-col">
            ...
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-wider">Customize Module Access</h3>
              <p className="text-[11px] text-zinc-450 mt-1 font-semibold">Enable or disable specific features for the {profile.role} role</p>
            </div>
            ...
            <div className="overflow-y-auto flex-1 pr-1 grid grid-cols-2 gap-3 text-xs font-semibold py-2">
              {modules.map((mod) => (
                ...
                >
                  <span className="uppercase tracking-wider text-[10px] font-black">{mod.name}</span>
