  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fadeIn pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white uppercase tracking-wider">Inventory Management</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {INVENTORY_STATS.map((stat, idx) => {
          ...
          return (
            ...
            >
              {/* Left Square Icon */}
              <div className={`h-11 w-11 rounded-xl flex items-center justify-center shrink-0 ${stat.iconColor}`}>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3.5">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full bg-[#121214] border border-zinc-800 rounded-2xl py-3 pl-11 pr-4 text-xs sm:text-sm text-white placeholder-zinc-550 focus:outline-none focus:border-orange-500 transition-colors"
          />
        </div>

        <button className="w-full sm:w-auto px-5 py-3 bg-orange-500 hover:bg-orange-600 rounded-2xl text-xs font-black uppercase tracking-wider text-white flex items-center justify-center space-x-1.5 transition-all shadow-md shadow-orange-500/10 cursor-pointer">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[#343435] text-[10px] font-black text-zinc-500 uppercase tracking-widest">
                ...
                <th className="pb-3.5">Min Stock</th>
                <th className="pb-3.5">Price/Unit</th>
            ...
            <tbody className="divide-y divide-[#343435] text-xs">
              {filteredItems.map((item) => (
                <tr key={item.id} className="hover:bg-zinc-900/10 group">
                  ...
                  <td className="py-4 font-bold text-zinc-500">{item.category}</td>
                  <td className="py-4 font-bold text-white">{item.quantity}</td>
                  <td className="py-4 font-bold text-zinc-400">{item.minStock}</td>
                  <td className="py-4 font-black text-orange-500">{item.price}</td>
                  <td className="py-4">
                    <span className={`
                      px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase border
                      ${item.status === "In Stock" && "bg-emerald-500/10 text-emerald-450 border-emerald-500/20"}
                      ${item.status === "Low Stock" && "bg-amber-500/10 text-amber-450 border-amber-500/20"}
                      ${item.status === "Out Of Stock" && "bg-red-500/10 text-red-450 border-red-500/20"}
                    `}>
                      {item.status}
                    </span>
                  </td>
                  <td className="py-4 text-right">
                    <button 
                      ...
                      className="px-4 py-1.5 bg-zinc-950/20 hover:bg-zinc-900 border border-zinc-800 hover:border-orange-500 text-zinc-400 hover:text-orange-550 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all flex items-center space-x-1 ml-auto cursor-pointer"
                    >
