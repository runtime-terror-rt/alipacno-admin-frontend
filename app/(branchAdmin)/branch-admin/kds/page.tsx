...
export default function KDSPage() {
  ...
  const KDS_COLUMNS = [
    ...
    { key: "preparing", label: "Preparing", buttonText: "Mark as Ready", color: "bg-emerald-500 hover:bg-emerald-600 text-white" },
    { key: "delayed", label: "Delayed:", buttonText: "Complete", color: "bg-zinc-800 hover:bg-zinc-750 text-zinc-300 border border-zinc-700" }
  ];
  ...
  // State transitions
  const handleStageTransition = (orderId: string, currentStatus: KDSOrder["status"]) => {
    if (currentStatus === "new") {
      setOrders(orders.map(o => o.id === orderId ? { ...o, status: "preparing" } : o));
    } else if (currentStatus === "preparing") {
      setOrders(orders.map(o => o.id === orderId ? { ...o, status: "delayed", timer: "20:00" } : o));
    } else {
              ...
              <div className="space-y-4">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        {/* Clock Counter */}
                        <div className={`
                          flex items-center space-x-1 px-2.5 py-0.5 rounded-lg text-[10px] font-black border
                          ${isDelayedCol 
                      ...
                      {/* Items loop */}
                      <div className="space-y-3.5 border-t border-b border-zinc-800/40 py-3">
                        {order.items.map((item, idx) => (
                          <div key={idx} className="text-xs">
                            <div className="flex items-start">
                              <span className="text-orange-500 font-black mr-2">{item.quantity}x</span>
                      ...
                      <button
                        onClick={() => handleStageTransition(order.id, order.status)}
                      ...
                      >
                        ...
                        {col.key === "preparing" && <Check className="h-3.5 w-3.5 stroke-[3px]" />}
                        {col.key === "delayed" && <CheckCircle2 className="h-3.5 w-3.5" />}
                ...
                {colOrders.length === 0 && (
                  <div className="h-36 border border-dashed border-zinc-800/60 rounded-2xl flex flex-col items-center justify-center text-center p-4">
                    <span className="text-[10px] font-bold text-zinc-650 uppercase tracking-widest">No Active Work</span>
                  </div>
                )}
              </div>
            </div>
