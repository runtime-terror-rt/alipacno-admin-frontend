    ...
    <div className="space-y-6 sm:space-y-8 animate-fadeIn pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex bg-[#121214] border border-[#343435] rounded-xl p-1">
            {[
              "Today",
              "Week",
              "Month",
              ...
              return (
                <button
                  ...
                  className={`
                    px-3 py-1.5 rounded-lg text-[10px] sm:text-xs font-black uppercase tracking-wider transition cursor-pointer
                    ${
                      isActive
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {INCOME_STATS.map((group, groupIdx) => {
          ...
          let iconBg = "bg-emerald-500/10 border-emerald-500/20 text-emerald-500";
          if (group.iconName === "package") iconBg = "bg-blue-500/10 border-blue-500/20 text-blue-500";
          if (group.iconName === "users") iconBg = "bg-purple-500/10 border-purple-500/20 text-purple-500";
          if (group.iconName === "clock") iconBg = "bg-amber-500/10 border-amber-500/20 text-amber-500";

          return (
            <div
              key={groupIdx}
              className="bg-[#121214] border border-[#343435] rounded-2xl p-5 relative overflow-hidden flex flex-col justify-between space-y-4"
              style={{
                backgroundImage:
                  "radial-gradient(circle at 95% 50%, rgba(204, 166, 147, 0.16) 0%, rgba(204, 166, 147, 0.04) 45%, transparent 75%)"
              }}
            >
              {/* Card Header Label */}
              ...
              <div className="space-y-2.5 pt-1">
                {group.metrics.map((metric, metricIdx) => (
                  <div key={metricIdx} className="flex justify-between items-end border-b border-[#343435] pb-1.5 last:border-b-0 last:pb-0">
                    <div>
                      <span className="block text-[9px] font-black text-zinc-550 uppercase tracking-wider">
                        {metric.label}
                      </span>
                      <span className="block text-base font-black text-white mt-1 leading-none">
                        {metric.value}
                      </span>
                    </div>

                    {/* Change indicator pill */}
                    <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-450 border border-emerald-500/20 text-[9px] font-black uppercase leading-none">
                      {metric.change}
                    </span>
                  </div>
                ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 bg-[#121214] border border-[#343435] rounded-2xl p-5 space-y-4">
            ...
            {TOP_PRODUCTS.map((prod) => (
              <div key={prod.name} className="space-y-2">
                <div className="flex justify-between items-center text-xs font-bold">
                  <div>
                    <span className="text-white block font-black">{prod.name}</span>
                ...
                <div className="w-full h-2 rounded-full bg-zinc-900 overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-orange-555 to-amber-500 rounded-full transition-all duration-1000"
        ...
        <div className="lg:col-span-2 bg-[#121214] border border-[#343435] rounded-2xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
          ...
          <div className="space-y-5 pt-1">
                ...
                <div key={method.name} className="space-y-2">
                  <div className="flex justify-between items-center text-xs font-bold">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-white font-black">{method.name}</span>
                    </div>
            ...
            <div className="pt-4 border-t border-zinc-850/60 flex justify-between items-center text-xs font-black">
              <span className="text-zinc-400 font-bold uppercase tracking-wider">Total Processed</span>
      ...
      <div className="bg-[#121214] border border-[#343435] rounded-2xl p-5 space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-zinc-800/60 text-[10px] font-black text-zinc-500 uppercase tracking-widest">
                ...
                <th className="pb-3.5">Status (mins)</th>
                <th className="pb-3.5">Revenue</th>
                <th className="pb-3.5 text-right">Performance</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#343435] text-xs">
              {HOURLY_PERFORMANCE.map((log, idx) => {
                // Style Status pills dynamically
                let statusPill = "bg-emerald-500/10 text-emerald-450 border border-emerald-500/20";
                if (log.status === "Preparing") {
                  statusPill = "bg-amber-500/10 text-amber-500 border border-amber-500/20";
                }
                if (log.status === "On Delivery") {
                  statusPill = "bg-blue-500/10 text-blue-400 border border-blue-500/20";
                }
                if (log.status === "Cancel") {
                  statusPill = "bg-rose-500/10 text-rose-500 border border-rose-500/20";
                }

                return (
                  <tr key={idx} className="hover:bg-zinc-900/10">
                    <td className="py-4 font-black text-orange-450">
                      {log.id}
                    </td>
                    ...
                    <td className="py-4">
                      <div className="flex items-center space-x-1.5">
                        <span className="px-2 py-0.5 rounded bg-zinc-900 border border-[#343435] text-[10px] text-zinc-400 font-bold uppercase">
                          {log.payment}
                        </span>
                      </div>
                    </td>
                    <td className="py-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase ${statusPill}`}>
                        {log.status}
                      </span>
                    </td>
                    ...
                    <td className="py-4 text-right">
                      {/* Micro bar-chart indicator inside table */}
                      <div className="flex items-center justify-end space-x-2">
                        <div className="w-20 h-1.5 rounded-full bg-zinc-900 overflow-hidden shrink-0">
                          <div
